  ...
  if(input.startsWith("rm -rf") && input.trim() !== "rm -rf") {
    if (isSudo) {
      if(input === "rm -rf /root" && !bareMode) {
        ...
        } else if (input === "rm -rf /root" && bareMode) {
          writeLines(["there's no more /root folder.", "<br>"])
        } else {
          if(bareMode) {
...
function passwordHandler() {
  if (passwordCounter === 2) {
    ...
    writeLines(["<br>", "DO YOU THINK YOU ARE GOOD AT THIS?", "PERMISSION NOT GRANTED.", "<br>"])
  ...
  if (PASSWORD_INPUT.value === SUDO_PASSWORD) {
    ...
    writeLines(["<br>", "PERMISSION GRANTED.","WELCOME BACK ADMIN!!", "If u'r a guest Don't Try <span class='command'>'rm -rf'</span>", "<br>"])
    revertPasswordChanges();
    isSudo = true;
    return
  } else {
